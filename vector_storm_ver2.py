...
#parameter
learning_rate = 0.05	 #tf.constant(0.001)
momentum = 0.001
...
class Storm_prediction(Annealer):
	
	def __init__(self, state):#, distance):
		#self.distance = distance;
		super(Storm_prediction, self).__init__(state);
	
	# make a new solution.
	def move(self):
		sess.run(optimizer, feed_dict= {x: train_x, y: train_y})
		updated_weights = sess.run(weights)
		updated_biases = sess.run(biases)
		
		self.state['weights'] = updated_weights
		self.state['biases'] = updated_biases
	
	# compute cost of current state
	def energy(self):
		e = sess.run(cost, feed_dict= {x: train_x, y: train_y})
		return e*100;
	
	
	
	
init_state = sess.run({'weights': weights, 'biases': biases})
storm_predict = Storm_prediction(init_state)

#print (init_state)

storm_predict.steps = 200000
storm_predict.Tmax = 250.0
storm_predict.Tmin = 2
storm_predict.updates = 10

storm_predict.copy_strategy = "method"
state, e = storm_predict.anneal()
...
with tf.Session() as sess:
	...
	for epoch in range(1):
		...
		for i in range(n_cluster_test):		# file_count
			...
			# run optimizer op
			a = sess.run(weights['h1'])
			#print (a)
			
			sess.run(optimizer, feed_dict= {x: train_x, y: train_y})
			b = sess.run(weights['h1'])
			#print (b)
			#print (a==b)
